...
namespace DrawGrid
{
    public static class GridDrawer
    {
        ...
        private static byte[] DrawSquareGrid(byte[] imageInput, Grid grid)
        {
            ...
            {
                // Draw vertical lines
                for (int x = 0; x < bitmap.Width; x += grid.Size)
                {
                    g.DrawLine(grid.Line, x, 0, x, bitmap.Height);
                }

                // Draw horizontal lines
                for (int y = 0; y < bitmap.Height; y += grid.Size)
                {
        ...
        private static byte[] DrawHaxagoneGrid(byte[] imageInput, Grid grid)
        {
            ...
            using (Graphics g = Graphics.FromImage(bitmap))
            {
                ...
                int numRows = (grid.Type == GridType.HexagoneX) ? (bitmap.Height / grid.Size) + 1 : (int)(bitmap.Height / (3 * (grid.Size / 2 / Math.Sqrt(3)))) + 1;
                int numCols = (grid.Type == GridType.HexagoneX) ? (int)(bitmap.Width / (3 * (grid.Size / 2 / Math.Sqrt(3)))) + 1 : (bitmap.Width / grid.Size) + 1;
                ...
                for (int row = 0; row < numRows; row++)
                {
                    for (int col = 0; col < numCols; col++)
                    {
                        ...
                        PointF[] points = GetHexagonPoints(grid.Type, grid.Size, row, col);
        ...
        [method: SupportedOSPlatform("windows")]
        private static PointF[] GetHexagonPoints(GridType type, float hexagoneHeight, float row, float col)
        {
            // Start with the leftmost corner of the upper left hexagon.
            float hexagoneWidth = HexWidth(hexagoneHeight);
            float y = hexagoneHeight / 2;
            float x = 0;
            ...
            if (col % 2 == 1) y += hexagoneHeight / 2;

            // Move over for the column number.
            x += col * (hexagoneWidth * 0.75f);

            if(type == GridType.HexagoneX)
            {
            ...
            else
            {
                ...
                {
                new PointF(y, x),
                new PointF(y - hexagoneHeight / 2, x + hexagoneWidth * 0.25f),
                new PointF(y - hexagoneHeight / 2, x + hexagoneWidth * 0.75f),
                new PointF(y, x + hexagoneWidth),
                new PointF(y + hexagoneHeight / 2, x + hexagoneWidth * 0.75f),
                new PointF(y + hexagoneHeight / 2, x + hexagoneWidth * 0.25f),
                };
            }
